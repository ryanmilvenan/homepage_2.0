...
module.exports = function(socket) {
    ...
    socket.on('sources:retrieve', function() {
        ...
        if(session && session.loggedIn) {
            db.User.findOne(session.user, function(err, doc) {
    ...
    socket.on('client:create-user', function(data) {
        ...
        db.User.findOne({username: user}, function(err, doc) {
            ...
            if(doc && doc.username == user) {
                ...
                var Tab = db.Tab;
                var User = db.User;
                db.NewsSource.find({username: "default"}, function(err, docs) {
                    var newTab = new Tab({title: "Home"});;
                    docs.forEach(function(doc) {
                        newTab.sources.push(doc);
                    })
