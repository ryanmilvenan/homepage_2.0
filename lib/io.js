...
module.exports = function(socket) {
    ...
    socket.on('sources:retrieve', function() {
        ...
        if(session && session.loggedIn) {
            db.User.findOne({username: session.user}, function(err, doc) {
    ...
    socket.on('sources:rename', function(source) {
        ...
        db.NewsSource.update( { sourceID: sourceID }, { $set: { 'title': title } } , function(err, doc) {
            if(err) return console.error(err);
        });
    ...
    socket.on('sources:new', function(data) {
        ...
        if(verified) {
            ...
            db.User.findOne({username: user}, function(err, userDoc) {
                if(err) return console.error(err);
                var tabIdx = tab.idx;
                var sourceIdx = userDoc.tabs[tabIdx].sources.length;
                var newSource = new NewsSource({url: url, title: title, username:user, tabIdx:tabIdx, sourceIdx:sourceIdx, sourceID: sourceID});
    ...
    socket.on('tab:new', function(data) {
        ...
        var Tab = db.Tab;
        var newTab = new Tab({title:title, idx:tabIdx});
                    ...
                    if(isMatch) {
                        ...
                        if(session) {
                            ...
                            session.save(function(err) {
                                if(err) return console.err(err);
                                db.Session.findOne(user, function(err, doc) {
            ...
            if(doc && doc.username == user) {
                ...
                db.User.findOne({username: "default"}, function(err, doc) {
                    ...
                    defaultSources.forEach(function(source) {
                        ...
                        var tabIdx = source.tabIdx;
                        var sourceIdx = source.sourceIdx;
                        var newSource = new NewsSource({url: url, title: title, username:user, tabIdx:tabIdx, sourceIdx:sourceIdx, sourceID: sourceID});
